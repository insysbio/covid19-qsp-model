...
% Compartments
nameless.compartment.Vol_alv = addcompartment(nameless_model, 'Vol_alv', 'ConstantCapacity', false, 'Capacity', 0.014, 'CapacityUnits', 'litre', 'Notes', '<p>calculated as (width of linning layer)<em>(lunf surface area) = (0.1 - 0.3 um)</em>(66 - 75 m)</p>
', 'Tag', '');

% Species
nameless.species.PC = addspecies(nameless.compartment.Vol_alv, 'PC', 'ConstantAmount', false, 'InitialAmount', 2643000000, 'InitialAmountUnits', 'kiloitem/litre', 'BoundaryCondition', false, 'Notes', '<p>calculated on the basis of total type II pneumocytes number per lungs (see ref) and volume of lung surfactant equal to 0.014L</p>
', 'Tag', '');
...
nameless.species.PC_hs_ss = addspecies(nameless.compartment.Vol_alv, 'PC_hs_ss', 'ConstantAmount', false, 'InitialAmount', 2643000000, 'InitialAmountUnits', 'kiloitem/litre', 'BoundaryCondition', true, 'Notes', '<p>estimate for type II pneumocytes was taken</p>
', 'Tag', '');
nameless.species.ACE2_pc = addspecies(nameless.compartment.Vol_alv, 'ACE2_pc', 'ConstantAmount', false, 'InitialAmount', 615, 'InitialAmountUnits', 'picomole', 'BoundaryCondition', false, 'Notes', '<p>based on 1e4 ACE2 per cell</p>
', 'Tag', '');
nameless.species.ACE2_pc_hs_ss = addspecies(nameless.compartment.Vol_alv, 'ACE2_pc_hs_ss', 'ConstantAmount', false, 'InitialAmount', 615, 'InitialAmountUnits', 'picomole', 'BoundaryCondition', true, 'Notes', '<p>based on 1e4 ACE2 per cell and total PC II number in lungs equal to 37e9 [7103258]</p>
', 'Tag', '');
...
nameless.species.COV = addspecies(nameless.compartment.Vol_alv, 'COV', 'ConstantAmount', false, 'InitialAmount', 0.0018, 'InitialAmountUnits', 'picomole/litre', 'BoundaryCondition', false, 'Notes', '', 'Tag', '');
...
nameless.species.anti_Ab = addspecies(nameless.compartment.Vol_alv, 'anti_Ab', 'ConstantAmount', false, 'InitialAmountUnits', 'picomole/litre', 'BoundaryCondition', true, 'Notes', '<p>hill like expression describes general kinetic pattern observed for Spike specific IgG and IgM in [32350462]</p>
', 'Tag', '');
...
nameless.parameter.switch_ir = addparameter(nameless_model, 'switch_ir', 'ConstantValue', false, 'Value', 0, 'ValueUnits', 'dimensionless', 'Notes', '<p>assumed</p>
', 'Tag', '');
...
nameless.parameter.k_apo_pc = addparameter(nameless_model, 'k_apo_pc', 'ConstantValue', true, 'Value', 0.00082, 'ValueUnits', '1/hour', 'Notes', '<p>the value was estimated from the HS submodel at steady state. Indeed, taking into account that (i) PC2 is precursors of PC1 [9227501], (ii) life span of PC1 is about 120 days [20616357], (iii) ratio of PC1/PC2 at steady state = 19/37 [7103258] we represented lung cell  dynamics system as follows: --(Vol_alv<em>k_mat_pc) --&gt; PC2 --(Vol_alv</em>k_dif_pc<em>PC2)--&gt; PC1--(Vol_alv</em>k_apo_pc1<em>PC1)--&gt;. Then consider the system at steady state and assuming that lifespan (T_ls) corresponds to time required for 99% of cell die we have come to following formulas: (a) k_apo_pc1 = ln(1/0.01)/(120 days) = 1.6e-3 1/h, (b) k_dif_pc = k_apo_pc1</em>PC1_ss/PC2_ss = 8.2e-4 1/h, (c) k_mat_pc = k_dif_pc*PC2_ss. Since  PC2 to PC1 transition was not considered in the version of the model we can assume that k_apo_pc = k_dif_pc</p>
', 'Tag', '');
nameless.parameter.kbase_apo_vpc = addparameter(nameless_model, 'kbase_apo_vpc', 'ConstantValue', true, 'Value', 0.00082, 'ValueUnits', '1/hour', 'Notes', '<p>the value was estimated from [PMID 18487354] only</p>
', 'Tag', '');
nameless.parameter.kbase_tran_pc_ipc = addparameter(nameless_model, 'kbase_tran_pc_ipc', 'ConstantValue', true, 'Value', 4, 'ValueUnits', '1/picomole*litre/hour', 'Notes', '<p>fitted to describe in vivo viral load dynamics</p>
', 'Tag', '');
nameless.parameter.kbase_tran_ipc_vpc = addparameter(nameless_model, 'kbase_tran_ipc_vpc', 'ConstantValue', true, 'Value', 4, 'ValueUnits', '1/picomole*litre/hour', 'Notes', '<p>fitted to describe in vivo viral load dynamics</p>
', 'Tag', '');
nameless.parameter.k_shed_ace2_pc = addparameter(nameless_model, 'k_shed_ace2_pc', 'ConstantValue', true, 'Value', 1.8, 'ValueUnits', '1/hour', 'Notes', '<p>rate constant of shedding catalysed by ADAM17/TACE = 5e-4 1/s</p>
', 'Tag', '');
nameless.parameter.k_off_cov_ace2 = addparameter(nameless_model, 'k_off_cov_ace2', 'ConstantValue', true, 'Value', 23.56, 'ValueUnits', '1/hour', 'Notes', '<p>k_off for viral Spike receptor binding domain (RBD) and ACE2 receptor</p>
', 'Tag', '');
nameless.parameter.Num_sp_per_cov = addparameter(nameless_model, 'Num_sp_per_cov', 'ConstantValue', true, 'Value', 150, 'ValueUnits', 'dimensionless', 'Notes', '<p>Basing on estimate  that virion diameter=120-160 nm and Spike height=20nm we have come to following number of Spikes per virion = 4<em>3.14</em>(70 nm)^2/(3.14*(10 nm)^2)  = 4*49 = 196. Assuming that &quot;dynamic&quot; diameter of Spike is little bit more we have assumed that there are about  150 spike proteins per 1 virion.</p>
', 'Tag', '');
nameless.parameter.Kd_spike_ace2 = addparameter(nameless_model, 'Kd_spike_ace2', 'ConstantValue', true, 'Value', 4670, 'ValueUnits', 'picomole/litre', 'Notes', '<p>Kd for viral Spike receptor binding domain (RBD) and ACE2</p>
', 'Tag', '');
nameless.parameter.k_ent_cov_ipc = addparameter(nameless_model, 'k_ent_cov_ipc', 'ConstantValue', true, 'Value', 0.936, 'ValueUnits', '1/hour', 'Notes', '<p>estimated on the basis of rate constant of endocytosis for Influenza virus</p>
', 'Tag', '');
nameless.parameter.k_rel_cov_vpc = addparameter(nameless_model, 'k_rel_cov_vpc', 'ConstantValue', true, 'Value', 2, 'ValueUnits', '1/hour', 'Notes', '<p>fitted to describe in vivo viral load dynamics averaged across 9 patients</p>
', 'Tag', '');
nameless.parameter.k_unc_cov_vpc = addparameter(nameless_model, 'k_unc_cov_vpc', 'ConstantValue', true, 'Value', 14, 'ValueUnits', '1/hour', 'Notes', '<p>estimated on the basis of rate constant of endocytosis for Influenza virus: k_end-cyt_Vend in original paper describing release from endosome/uncoating</p>
', 'Tag', '');
nameless.parameter.k_rep_cov_rna_vpc = addparameter(nameless_model, 'k_rep_cov_rna_vpc', 'ConstantValue', true, 'Value', 0.2, 'ValueUnits', '1/hour', 'Notes', '<p>fitted to describe in vivo viral load dynamics averaged across 9 patients</p>
', 'Tag', '');
nameless.parameter.EC50_rep_cov_rna_vpc = addparameter(nameless_model, 'EC50_rep_cov_rna_vpc', 'ConstantValue', true, 'Value', 9992, 'ValueUnits', 'picomole/litre', 'Notes', '<p>it was proposed that a host cell can produce about 8000 (item/cell) virions of Influenza virus before it dies [15384040]. So we have calculated the EC50 basing on the value; taking into account that Vol_pc = 1.33e-12 L/cell we can obtain estimate in pM: EC50 = 8e3 /NA_pmole/Vol_pc = 8e3/6.02e11/1.33e-12 = 9992 pM</p>
', 'Tag', '');
nameless.parameter.k_ass_cov_vpc = addparameter(nameless_model, 'k_ass_cov_vpc', 'ConstantValue', true, 'Value', 0.39, 'ValueUnits', '1/hour', 'Notes', '<p>fitted to describe in vivo viral load dynamics averaged across 9 patients</p>
', 'Tag', '');
nameless.parameter.k_deg_cov = addparameter(nameless_model, 'k_deg_cov', 'ConstantValue', true, 'Value', 0.17, 'ValueUnits', '1/hour', 'Notes', '<p>COV half-life on different surfaces and aerosole is varied from 1 to 8 hours. We have chosen half-life equal to 4 hour. So, rate constant is equal to ln2/4 = 0.17 1/h</p>
', 'Tag', '');
...
', 'Tag', '');
nameless.parameter.k_deg_cov_rna = addparameter(nameless_model, 'k_deg_cov_rna', 'ConstantValue', true, 'Value', 17, 'ValueUnits', '1/hour', 'Notes', '<p>assumed to be equal to COV half-life equal to 4 hour. So, rate constant is equal to ln2/4 = 0.17 1/h</p>
', 'Tag', '');
nameless.parameter.T_sw_ir = addparameter(nameless_model, 'T_sw_ir', 'ConstantValue', true, 'Value', 50, 'ValueUnits', 'hour', 'Notes', '<p>assumed</p>
', 'Tag', '');
nameless.parameter.ET50_ir = addparameter(nameless_model, 'ET50_ir', 'ConstantValue', true, 'Value', 190, 'ValueUnits', 'hour', 'Notes', '<p>assumed</p>
', 'Tag', '');
nameless.parameter.Emax_ir_apo = addparameter(nameless_model, 'Emax_ir_apo', 'ConstantValue', true, 'Value', 700, 'ValueUnits', 'dimensionless', 'Notes', '<p>assumed</p>
', 'Tag', '');
nameless.parameter.n_ir = addparameter(nameless_model, 'n_ir', 'ConstantValue', true, 'Value', 5, 'ValueUnits', 'dimensionless', 'Notes', '<p>assumed</p>
', 'Tag', '');
nameless.parameter.Kd_anti_Ab = addparameter(nameless_model, 'Kd_anti_Ab', 'ConstantValue', true, 'Value', 14300, 'ValueUnits', 'picomole/litre', 'Notes', '<p>assumed to be equal to tat presented in the paper for neutralizing mAb</p>
', 'Tag', '');
nameless.parameter.anti_Ab_max = addparameter(nameless_model, 'anti_Ab_max', 'ConstantValue', true, 'Value', 941000, 'ValueUnits', 'picomole/litre', 'Notes', '<p>For Protective Antigen (PA) for Anthrax vaccine it was measured that concentration of Anti-PA specific IgG in serum is 141.2 ug/mL [15358653]. Taking into account Mr_IgG = 150 kDa, one obtains 141.2 ug/mL =</p>
', 'Tag', '');
nameless.parameter.Vol_pc = addparameter(nameless_model, 'Vol_pc', 'ConstantValue', true, 'Value', 1.33e-12, 'ValueUnits', 'litre/item', 'Notes', '<p>averaged between type I and II pneumocytes (moved to Const)</p>
', 'Tag', '');
...
nameless.reaction.V_tran_pc_ipc = addreaction(nameless_model, 'null -> null', 'Name', 'V_tran_pc_ipc', 'Active', true, 'Reversible', true, 'ReactionRate', 'Vol_alv * k_tran_pc_ipc * PC', 'Notes', '', 'Tag', '');
...
nameless.reaction.V_tran_ipc_vpc = addreaction(nameless_model, 'null -> null', 'Name', 'V_tran_ipc_vpc', 'Active', true, 'Reversible', true, 'ReactionRate', 'Vol_alv * k_tran_ipc_vpc * iPC', 'Notes', '', 'Tag', '');
...
% Time Events
nameless.event.viral_load = addevent(nameless_model, 'time >= T_sw_ir && (time - T_sw_ir) /  - floor((time - T_sw_ir) /  + 0.5) >= 0', {'switch_ir = 1', }, 'Active', true, 'Notes', '', 'Tag', '');
